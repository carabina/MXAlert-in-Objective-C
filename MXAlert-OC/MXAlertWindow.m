...
@implementation MXAlertWindow
+ (instancetype)defaultWindow {
    static MXAlertWindow *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[MXAlertWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
    });
    return instance;
}
...
- (void)bringWindowToTop:(NSNotification *)notification {
    if ([notification object] == nil || ![[notification object] isKindOfClass:[MXAlertWindow class]]) {
        [[MXAlertWindow defaultWindow] setHidden:YES];
        [[MXAlertWindow defaultWindow] setHidden:NO];
    }
}
...
- (void)handleRotate:(UIInterfaceOrientation)orientation {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        MXAlertOperation *alert = [[MXAlertManager defaultManager] currentAlert];
        if (alert != nil) {
            [[alert alertView] updateView];
        }
    });
}
